...
netdataDashboard.submenu = {
    ...
    'cpu.softnet_stat': {
        ...
        info: function (os) {
            if (os === 'linux')
                return '与CPU接收工作相关的每CPU核心SoftIRQ的统计信息。 Total for all CPU cores can be found at <a href="#menu_system_submenu_softnet_stat">System / softnet statistics</a>. <b>processed</b> states the number of packets processed, <b>dropped</b> is the number packets dropped because the network device backlog was full (to fix them on Linux use <code>sysctl</code> to increase <code>net.core.netdev_max_backlog</code>), <b>squeezed</b> is the number of packets dropped because the network device budget ran out (to fix them on Linux use <code>sysctl</code> to increase <code>net.core.netdev_budget</code> and/or <code>net.core.netdev_budget_usecs</code>). More information about identifying and troubleshooting network driver related issues can be found at <a href="https://access.redhat.com/sites/default/files/attachments/20150325_network_performance_tuning.pdf" target="_blank">Red Hat Enterprise Linux Network Performance Tuning Guide</a>.';
            else
...
netdataDashboard.context = {
    'system.cpu': {
        info: function (os) {
            void(os);
            return 'CPU使用率(所有核心).这里100% 表示已经没有空闲CPU资源可用。你可以从 <a href="#menu_cpu">CPU总</a> 获取每个核心的信息，从 <a href="#menu_apps">应用监控</a> 获取每个应用的信息。' + netdataDashboard.sparkline('<br/>请关注 <b>iowait</b> ', 'system.cpu', 'iowait', '%', '. 如果它一直很高，那么您的磁盘就会成为瓶颈，它们会降低系统速度.') + netdataDashboard.sparkline('<br/>还有一个值得监控的重要指标是： <b>softirq</b> ', 'system.cpu', 'softirq', '%', '. 持续高百分比的softirq可能表示网络驱动程序问题。');
        },
        valueRange: "[0, 100]"
    },
    ...
    'system.interrupts': {
        info: 'CPU中断详情。 在 <a href="#menu_cpu">CPU总</a> 面板, 可以分析每个CPU的情况。'
    },

    'system.softirqs': {
        info: 'CPU软终端详情. 在 <a href="#menu_cpu">CPU总</a> 面板, 可以分析每个CPU的情况。'
    },

    'system.processes': {
        info: '系统进程。<b>Running</b> 是运行在CPU的进程，<b>Blocked</b> 是正在等待进入CPU的进程, 因为他们在等待磁盘IO'
    },
    ...
    'mem.pgfaults': {
        info: 'A <a href="https://en.wikipedia.org/wiki/Page_fault" target="_blank">page fault</a> 当一个正在运行的程序访问映射到虚拟地址空间但实际上没有加载到主存储器中的内存页时，由计算机硬件引发的一种称为陷阱的中断。如果在生成故障时将页面加载到内存中，但未在内存管理单元中将其标记为已加载到内存中，则称其为<b>minor</ b>或软页面错误。当系统需要从磁盘或交换内存加载内存页时，会产生<b>major</b> 错误。'
    },
    ...
    'mem.available': {
        info: '可用内存由内核估算，作为用户空间进程可以使用的RAM量，而不会导致交换.'
    },
    ...
    'mem.kernel': {
        info: '内核使用的内存总量。 <b>Slab</b> 是内核用于缓存数据结构以供自己使用的内存量。 <b>KernelStack</b> 是为内核完成的每个任务分配的内存量。 <b>PageTables</b> 是专用于最低级别页表的内存量（页表用于将虚拟地址转换为物理内存地址）。<b>VmallocUsed</b> 是用作虚拟地址空间的内存量。'
    },
